...
coala:
  ...
  - Do not work on any issue without assignment. Someone else might be working
    on it already.
  - When in doubt, `talk to us <http://coala.io/chat>`_.
...
.. note::
    ...
    in a mergable state. We want you to learn and develope an eye for finding
...
to contact us on `gitter <https://coala.io/chat>`_, and we will help you!
...
`Newcomers issues <https://coala.io/new>`_.
...
.. seealso::
    ...
    `Writing Native bears <http://api.coala.io/en/latest/Developers/Writing_Native_Bears.html>`_
...
..note::
    ...
    Do not ask for reviews in the chat. Instead go to http://coala.io/review and
...
For more information about reviewing code, check out this
`link <http://coala.io/reviewing>`_.
...
Now all you have to do is wait, or let the other developers know on Gitter that
you have published your changes.

.. note::

    Do not tag the reviewers every time you push a change.
    They review PRs consistently whenever they have time!
...
send it again like this. If not, edit it and send it.
